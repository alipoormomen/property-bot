...
async def handle_callback_query(update: Update, context=None):
    """پردازش callback query از دکمه‌های inline"""
...
async def handle_edit_request(user_id: int, text: str, update: Update) -> bool:
    """پردازش درخواست ویرایش فیلد"""

    # الگو: "فیلد: مقدار" یا "فیلد = مقدار"
    ...
    if not field_key:
        await update.message.reply_text(
            ...
            "لطفاً از نام‌های استاندارد استفاده کنید."
        )
        return True

    # نرمال‌سازی مقدار
    if field_key in PRICE_FIELDS:
        normalized = normalize_price(new_value)
        if normalized:
        ...
        new_value = new_value.lower() in ["بله", "دارد", "داره", "آره", "اره", "yes", "true"]

    elif field_key in ["area", "rooms", "floor", "year_built"]:
        try:
            new_value = int(re.sub(r'\D', '', new_value))
        except ValueError:
            pass
    ...
    # نمایش پیام تایید با اطلاعات به‌روز شده
